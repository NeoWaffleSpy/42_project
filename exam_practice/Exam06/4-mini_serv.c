...
int		main(int ac, char **av) {
	...
	while(1) {
		...
		if (select(fdMax + 1, &readfds, &writefds, NULL, NULL))
			continue;
		for (int fdI = 0; fdI <= fdMax; fdI++) {
			if (FD_ISSET(fdI, &readfds) && fdI == sockfd) {
				int connfd = accept(sockfd, (struct sockaddr *)&clients, (socklen_t*)&len);
				if (!connfd)
					continue;
			...
			else if (FD_ISSET(fdI, &readfds) && fdI != sockfd) {
				int rec = recv(sockfd, bufferRead, 60000, 0);
				if (rec < 0) {
					...
					for (int i = 0, j = strlen(clients[fdI].msg); i <= fdMax; i++, j++) {
