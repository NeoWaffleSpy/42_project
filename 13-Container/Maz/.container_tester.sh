...
run_tests ()
{
	...
	clang++ -Werror -Wextra -Werror -std=c++98 -g3 -I . -I "$INCLUDES" -I "$INCLUDES"/EMPTY -I ../"$2" -I "$INCLUDES"/EMPTY/JUST_IN_CASE \
			mains/"$1"_main.cpp -o bin/"$1"_ft 2> logs/"$1"_ft.error.log
	clang++ -Werror -Wextra -Werror -std=c++98 -g3 -D STD -I . -I "$INCLUDES" -I "$INCLUDES"/EMPTY \
	...
	then
		...
		then
			...
			then
				if [[ "$OSTYPE" == "linux-gnu"* ]]
				then
					valgrind --leak-check=full --track-origins=yes ./bin/"$1"_ft > /dev/null 2> leak/"$1"_leak
